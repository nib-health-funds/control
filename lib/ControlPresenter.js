...
function ControlPresenter(options) {
  ...
  this.events         = options.event && Array.isArray(options.event) ? options.event : [options.event];
  ...
  this.inputView
    ...
    .on('click', function() {
      self.emit('click'); //FIXME - only proxy the event when something is listening for it
    })
    .on('blur', function() {
      self.emit('blur'); //FIXME - only proxy the event when something is listening for it
    })
  ;

  //listen for events which meed tp trigger `validate`
...
ControlPresenter.prototype.clear = function() {
  ...
  this.controlView.clearMessage();
  this.controlView.hideMessage();
  return this;
};
...
ControlPresenter.prototype.validate = function() {
  ...
  //set the loader to display when the filter/validate functions take longer than the current event loop
  var loadingTimeout = setTimeout(function() {
    self.controlView.setLoading(true);
  }, 0);
  ...
  self.filterChain.filter(theValue, function(theFilteredValue) {
    ...
    self.validatorChain.validate(theFilteredValue, function (err, valid, context) {
      ...
      self.controlView.setLoading(false);
