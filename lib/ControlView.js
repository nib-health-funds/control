...
var ControlView = require('view').extend({
  ...
  clearMessage: function() {
    ...
    if (typeof this.feedbackMessage.textContent !== 'undefined') {
      ...
      this.feedbackMessage.innerText = ''; //IE8
    }
  ...
  setMessage: function(type, text) {
    ...
    if (typeof this.feedbackMessage.textContent !== 'undefined') {
      ...
      this.feedbackMessage.innerText = text; //IE8
    }
  ...
  hideMessage: function(callback) {
    if (this.feedbackContainer) {
      transition(this.feedbackContainer, 'height', '0px', callback);
    } else if (callback) {
      callback();
    }
    return this;
  ...
  showMessage: function(callback) {
    if (this.feedbackContainer) {
      transition(this.feedbackContainer, 'height', 'auto', callback);
    } else if (callback) {
      callback();
    }
    return this;
